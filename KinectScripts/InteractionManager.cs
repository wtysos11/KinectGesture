...
public class InteractionManager : MonoBehaviour 
{
    ...
    //是否在屏幕上显示用手控制的鼠标，如果是，则下面的贴图也要被设置
	...
	void Update () 
	{
        ...
        // 如果kinectManager已经被初始化
		if(kinectManager && kinectManager.IsInitialized())
		{
			primaryUserID = kinectManager.GetUserIdByIndex(playerIndex);//playerIndex表示Kinect上检测到的人的序号
			
			if(primaryUserID != 0)
			{
				HandEventType handEvent = HandEventType.None;
				
				// get the left hand state 拿左手状态
				leftHandState = kinectManager.GetLeftHandState(primaryUserID);
				
				// check if the left hand is interacting 检查左手是否可交互
                ...
                //如果左手可交互，而且KinectManager可以与该对象进行交互
                // 判断isLeftHandINteracting，且通过左手的空间位置leftHandPos来确定左手在屏幕上的位置leftHandScreenPos
				   ...
				   kinectManager.GetJointTrackingState(primaryUserID, (int)KinectInterop.JointType.HandLeft) != KinectInterop.TrackingState.NotTracked)
				{
                    ...
                    //左手过去是否可交互，保存isLeftHandInteracting的状态。并且isLeftHanInteracting更新自己的状态
					...
					// check for left press 检查左手是否下压：通过z轴的变化
					isLeftHandPress = ((leftIboxRightTopFront.z - 0.1f) >= leftHandPos.z);
					
					// check for left hand click 检查左手是否点击
					float fClickDist = (leftHandPos - lastLeftHandPos).magnitude;

					if(allowHandClicks && !dragInProgress && isLeftHandInteracting && 
					   (fClickDist < KinectInterop.Constants.ClickMaxDistance))//fClickDist<ClickMaxDistance，且其他条件满足，则确定存在点击
					{
						if((Time.realtimeSinceStartup - lastLeftHandTime) >= KinectInterop.Constants.ClickStayDuration)//如果点击时间满足条件，进行点击
						{
							if(!isLeftHandClick)
							{
				...
				// process left hand 处理左手，用左手的状态leftHandState决定左手的活动lastLeftHandEvent
				...
				{
					if(controlMouseCursor && dragInProgress)//释放鼠标
					{
				...
				// if the hand is primary, set the cursor position 如果存在左手，根据手的位置设置鼠标坐标
				if(isLeftHandInteracting)//如果左手可交互，那么左手就存在
				{
