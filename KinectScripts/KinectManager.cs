...
//					}
	...
	void Update() 
	{
		if(kinectInitialized)
		{
			...
			{
				// update Kinect streams and structures 更新Kinect流和相关数据结构
				UpdateKinectStreams();
			}

			// process the data from Kinect streams 处理Kinect流
			ProcessKinectStreams();

			// update the avatars 更新Avatar 人体动作检测
...
//				}
				...
				// notify the gesture listeners about the new user 新用户加入，通知所有Listener新的用户
				foreach(KinectGestures.GestureListenerInterface listener in gestureListeners)
				{
					if(listener != null)
					{
...
//						else
			...
			for(int g = 0; g < listGestureSize; g++)
			{
					...
					!IsConflictingGestureInProgress(gestureData, ref gesturesData))
				{
					gestureManager.CheckForGesture(UserId, ref gestureData, Time.realtimeSinceStartup, 
						ref playerJointsPos, ref playerJointsTracked);//这里调用了
	...
	protected virtual bool CheckForCalibrationPose(Int64 UserId, int bodyIndex, KinectGestures.Gestures calibrationGesture)
	{
		...
		// estimate the gesture progess 调用2
		gestureManager.CheckForGesture(UserId, ref gestureData, Time.realtimeSinceStartup, 
